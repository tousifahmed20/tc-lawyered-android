...
> Note: `gradle/wrapper/gradle-wrapper.properties` pins Gradle 8.11.1. The binary
> `gradle-wrapper.jar` and the `gradlew`/`gradlew.bat` scripts are generated by
> Android Studio on first sync (or run `gradle wrapper` once with a local Gradle).
...
- [ ] Slice 5b — premium OpenAI TTS, protection-tips videos (YouTube key),
      glossary tooltips; commit the wrapper JAR + first real compile pass.
